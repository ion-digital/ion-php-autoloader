...
class PackageTest extends TestCase {
    ...
    private static function createRootDirectory(bool $createFileName = true) {
        
        return( realpath(__DIR__ . DIRECTORY_SEPARATOR . self::AUTO_LOADER_PROJECT_DIR) . DIRECTORY_SEPARATOR . ($createFileName ? self::ENTRY_FILENAME : '') );
    }
    
    private static function createPackage(string $project, bool $debug = null, bool $cache = null, array $loaders = null, bool $createVersion = true, bool $createFileName = true) {
...
//        echo "\n";
        ...
        //die(static::createRootDirectory());
        ...
        ], static::createRootDirectory($createFileName), $version, $debug, $cache, $loaders);                     

    }
    
//    public function testCreateDirectory() {
//        
//        $dir1 = static::createRootDirectory(true);
//        
//        
//        
//        $dir2 = static::createRootDirectory(false);
//        
//        die($dir2);
//    }
    ...
    public function testVersionAndName() {
        ...
        $package = self::createPackage(self::TEST_PACKAGE_PROJECT, true, false, [], true, true);
    ...
    public function testPathsForPackageWithFilename() {
        ...
        $packageWithFilename = self::createPackage(self::TEST_PACKAGE_PROJECT, true, false, [], true, true);

        $this->assertEquals(static::createRootDirectory(false), $packageWithFilename->getProjectRoot());        
        $this->assertEquals(static::createRootDirectory(true), $packageWithFilename->getProjectEntry());
    ...
    public function testPathsForPackageWithoutFilename() {    
        ...
        $packageWithoutFilename = self::createPackage(self::TEST_PACKAGE_PROJECT, true, false, [], true, false);
                
        $this->assertEquals(static::createRootDirectory(false), $packageWithoutFilename->getProjectRoot());        
        $this->assertNull($packageWithoutFilename->getProjectEntry());
    ...
    public function testDebug() {
        ...
        $package1 = self::createPackage(self::TEST_PACKAGE_PROJECT_7, true, false);     
        ...
        $package2 = self::createPackage(self::TEST_PACKAGE_PROJECT_8, false, false);      
    ...
    public function testLoadConfiguration() {
        ...
        $package =  self::createPackage(self::TEST_PACKAGE_PROJECT_10, null, false, null, false);  
        
        $this->assertEquals(true, file_exists($package->getProjectRoot() . Package::ION_PACKAGE_CONFIGURATION_FILENAME));
        $this->assertEquals(self::TEST_PACKAGE_PROJECT_10, $package->getProject());

        $this->assertEquals(true, $package->getConfiguration()->getSetting('bool_true'));
        $this->assertEquals(false, $package->getConfiguration()->getSetting('bool_false'));

        $this->assertEquals(true, $package->getConfiguration()->getSettingAsBool('bool_true'));
        $this->assertEquals(false, $package->getConfiguration()->getSettingAsBool('bool_false'));        
        
        $this->assertEquals(true, $package->getConfiguration()['bool_true']);
        $this->assertEquals(false, $package->getConfiguration()['bool_false']);
